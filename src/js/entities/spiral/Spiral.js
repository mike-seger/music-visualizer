...
export default class Spiral extends THREE.Object3D {
  ...
  update(deltaTime) {
    ...
    for (let i = 0; i < this.numParticles; i++) {
      const theta = (i / this.numParticles) * Math.PI * 2 * this.angle
      ...
      const audioRadius = radius + freqValue * this.intensity
      const x = Math.cos(theta) * audioRadius
      const y = Math.sin(theta) * audioRadius
      const z = timeValue * 0.3
      ...
      const normalizedRadius = radius / (this.a + this.b * Math.PI * 2 * this.angle)
      ...
      // Size grows from center (0.15) to outer (0.45)
      const size = 0.15 + normalizedRadius * 0.3
      sizes[i] = size
    }
