...
void animateGlobals()
{
    ...
    // Disabled for this app: keep the camera deterministic (time + audio only).
    vec2 click = vec2(0.0);
    ...
    // the camera farther away from the origin (more orbs in view). We can then slowly
    // rotate the crane around it's azimuth to explore the scene.
    g_camOrigin = vec3(0., 0.0 , -350.);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Smaller focal length => wider FOV => more orbs visible.
    float camFocalLengthScalar = mix(.25, 0.85, vignet);
