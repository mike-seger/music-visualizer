...
float mapScene(vec3 p, out vec2 cellId)
{
    ...
    for (int j = 0; j <= 1; j++) {
        for (int i = 0; i <= 1; i++) {
            ...
            // Audio reactivity: each X-column maps to a frequency band (bass → mids).
            // Three adjacent FFT bins are averaged so the movement isn't too nervous.
            float freqBin  = 0.02 + ((c.x + 0.5) / float(N)) * 0.48;
            float hw       = 0.025;
            float audio    = (texture(iChannel0, vec2(freqBin - hw, 0.25)).r +
                              texture(iChannel0, vec2(freqBin,       0.25)).r +
                              texture(iChannel0, vec2(freqBin + hw,  0.25)).r) / 3.0;
            // Square the value so only meaningful peaks cause visible lifts.
            float audioLift = 0.55 * audio * audio;

            // Local position relative to this cube center: slow bob + audio lift.
            vec3 q = p - vec3(center.x, bob + audioLift, center.y);
